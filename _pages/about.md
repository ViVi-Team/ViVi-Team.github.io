---
permalink: /
title: "ViVi Research Team"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

Welcome to the official website of **ViVi**, a non-profit research team dedicated to democratizing artificial intelligence for healthcare.

ViVi focuses on developing next-generation **Multilingual Multimodal Multipurpose models, datasets, and evaluation benchmarks** to support medical AI research. 

This website serves as a central platform to present our research activities, publications, and projects. 

---

Research Focus
======

Our current research directions include:

- Large Language Models for clinical text understanding and medical knowledge reasoning  
- Vision-Language Models for radiology report generation and multimodal learning  
- Medical image analysis (X-ray, CT, MRI, ultrasound)  
- Self-supervised and weakly-supervised learning for limited medical datasets  
- Trustworthy AI: interpretability, fairness, and robustness  
- AI-assisted clinical decision support systems  

We place particular emphasis on applications related to:

- Early detection and monitoring of **Alzheimer’s disease**  
- Diagnosis and prognosis of **lung diseases**, including pneumonia and other respiratory conditions  

Through these efforts, we aim to develop AI tools that can assist clinicians, reduce workload, and improve diagnostic accuracy in real clinical settings.

---

Our Team
======

ViVi is composed of a diverse and international group of:

- Undergraduate researchers  
- Master’s students  
- PhD candidates  
- Academic mentors and collaborators
- Industry and business mentors

Our members come from leading universities around the world, including institutions in Canada, US, Germany, etc... We foster a collaborative environment where researchers and clinicians work closely together, combining expertise in computer science, machine learning, biomedical engineering, and medicine.

![ViVi Research Team](/images/about-team.png)

We strongly believe that interdisciplinary collaboration is essential for impactful AI research.

---

Our Vision
======

Our long-term vision is to make AI a trustworthy partner in healthcare.

We aim to:

- Improve early disease detection  
- Support clinicians with intelligent decision tools  
- Enhance healthcare accessibility  
- Translate research into real-world medical impact  

By combining strong academic foundations with practical applications, ViVi strives to contribute meaningful solutions to global healthcare challenges.

---

Join Us
======

We welcome research collaborations, student internships, and partnerships with hospitals, universities, and industry.

If you are interested in medical AI, foundation models, or interdisciplinary healthcare research, we would be happy to connect and work together.

Together, we build AI for better medicine.
